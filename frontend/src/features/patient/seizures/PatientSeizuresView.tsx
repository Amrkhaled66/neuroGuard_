...
import PatientSeizuresHeader from "./components/PatientSeizuresHeader";
import PatientSeizuresMeaningCard from "./components/PatientSeizuresMeaningCard";

const PAGE_SIZE = 10;

export default function PatientSeizuresView() {
    ...
    return (
      <section className="app-surface rounded-3xl p-6 text-red-600">
        {analyticsQuery.error instanceof Error
          ...
          : "Failed to load seizure analytics."}
      </section>
    );
  }
    ...
    <section>
      ...
      <div className="mx-auto grid w-full grid-cols-1 gap-6 xl:grid-cols-[2fr_1fr]">
        <div className="flex min-w-0 flex-col gap-6">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-5">
            ...
            {isLoading && summaryCards.length === 0
              ? Array.from({ length: 5 }).map((_, index) => (
                  <SummaryCard key={index} title="" value="" subtitle="" isLoading />
                ))
              : null}
          </div>

          <SeizureTrendChart data={analytics?.trend ?? []} isLoading={isLoading} />
          ...
          <PatientSeizuresMeaningCard />
        </div>

        <div className="flex min-w-0 flex-col gap-6">
          <div className="app-surface rounded-3xl p-6">
            <h3 className="app-text-primary text-2xl font-bold">Pattern Analysis</h3>
          ...
          <IntensityDistributionCard
            data={analytics?.durationDistribution ?? { high: 0, med: 0, low: 0 }}
            isLoading={isLoading}
          />
        </div>
      </div>
